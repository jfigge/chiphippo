...
|-----|------|------------------|------------|
| 00  | [Project scaffold & build system](done/00-project-scaffold.md) | Repo layout, Makefile, `src/app`+`src/web` Electron shell, empty desk shell, `window.chiphippo` IPC bridge, lint/format/test/build/`make debug`, Apache-2.0 + license-header guard | — |
| 10  | [Infinite desk workspace](done/10-infinite-desk.md) | Pannable/zoomable infinite desk (camera transform, not scrollbars), dot grid, zoom controls, pure world↔screen geometry module, settings store + persisted viewport | 00 |
| 20  | [Breadboard model & tie-point addressing](done/20-breadboard-model.md) | Pure breadboard geometry/addressing/connectivity model for all three sizes (830/400/170), the desk-document store with autosave IPC, migrations stub, ipc-parity test | 00 |
| 30  | [Breadboard rendering & placement](done/30-breadboard-rendering.md) | Boards drawn (SVG per board, math hit-testing — no per-hole DOM) on the desk; add/drag/select/delete boards; hover any hole to see its address | 10, 20 |
| 40  | [Component framework & DIP chip placement](done/40-chip-placement.md) | Component/occupancy model, data-driven 74xx catalog (metadata + pinouts), searchable chip palette, DIP footprints straddling the trench, ghost preview, legal placement, pin tooltips | 30 |
| 50  | [Wires](done/50-wires.md) | Hole-to-hole wires (incl. cross-board), click-click wiring with rubber-band preview, colored sagging bezier rendering on an SVG overlay, select/delete | 40 |
| 60  | [Switches, LEDs & power sources](done/60-discrete-components.md) | Discrete parts in the same framework: slide switch, push button, LEDs (4 colors, polarized), and desk-level PSU bricks (3 V / 5 V / 12 V) with addressable terminals | 50 |
| 70  | [Netlist & connectivity inspector](done/70-netlist-and-inspector.md) | Pure union-find netlist over strips/rails/wires/switch-state; hover-to-highlight an entire electrical net across boards; net summary readout | 60 |
| 80  | [74xx behavioral library v1](done/80-ttl-chip-library.md) | Signal levels (H/L/Z/X), generic gate evaluator, behavioral defs + truth-table tests for the combinational gate wave (7400/02/04/08/10/11/20/27/30/32/86, 74125) | 40 |
| 90  | [Simulation engine v1](done/90-simulation-engine.md) | Rudimentary but real: power-rail seeding, chip VCC/GND power checks, iterative settle loop with warm start (SR latches work), conflict & oscillation detection, live switches, glowing LEDs, Run/Stop | 70, 80 |
| 100 | [Sequential logic & clocking](done/100-sequential-and-clocking.md) | Stateful edge-triggered chips (7473/74/75/76, 74107), MSI wave (74138/139/151/157/161/164/165/175/193), clock-source component, run/pause/single-step + speed control | 90 |
...
| 150 | [Schematic view](done/150-schematic-view.md) | A derived logical diagram — chip symbols + routed named nets + bus lines, deterministic auto-layout with nudge, live sim tint, shared probe | 70, 120, 130 |
| 160 | [Export to SVG / PNG / PDF](deferred/160-export-image-and-pdf.md) | Standalone vector/raster export of desk + schematic and a full build-package PDF (schematic + board + guide); fonts embedded, main-side write | 140, 150 |
| 170 | [Memory chips & wide DIPs](done/170-memory-chips-and-wide-dips.md) | DIP-24/28 footprints, a `memUnit` vocabulary, ROM/SRAM/EEPROM catalog defs; async read + reported writes over a run-volatile image, engine stays pure | 40, 80/100, 130 |
| 180 | [File-backed byte storage](done/180-file-backed-memory.md) | Memory bound to a real `.bin` via `mem:*` IPC; controller loads on Run, flushes writes atomically, binding persists in the doc | 170 |
| 190 | [Memory inspector / hex editor](done/190-memory-inspector.md) | Per-chip floating hex/ASCII window; edit-when-stopped, live-when-running, Intel HEX + `.bin` import/export, fill/goto, binding management | 170, 180 |
...
| 210 | [Logic analyzer & timing view](done/210-logic-analyzer.md) | A passive recorder over `chiphippo:sim-state`: net/bus channels, scrolling waveforms + hex bus lanes, cursors/Δ readout, exportable timing charts | 90/100, 120, 130, 170 |
...
| 250 | [The project IS the document](done/250-single-file-projects.md) | One `.chiphippo` file holds every desktop and every programmed ROM image — one dirty marker, one Save, one File menu; desktop Save As/Open become Export/Import; deletes the two-file-lifetime machinery (app-kept flags, orphan GC, eager write-through) that made a shared project's paths dangle | 180, 200, 240 |
| 260 | [AI circuit builder](done/260-ai-circuit-builder.md) | Describe a circuit in words and get a wired, **simulation-proven** design clip: the user's own API key (Anthropic or any OpenAI-compatible endpoint) behind `safeStorage` in main, the model emitting a coordinate-free **netlist** it cannot get geometry wrong in, and a pure compiler that places, routes, and proves the result through the real engine before offering it | 130, 200 |
| 270 | [Example circuits](done/270-example-circuits.md) | Every benchable 74xx part's demonstration bench shipped **inside** the app (`src/web/demos/<ref>.json`, written by the same `make demos` pass as `demos/`, so the two cannot drift), offered as a button on that part's pin-assignments window: one click lands a `<ref> example` desktop in the open project, centred and ready to Run | 100, 240, 250 |
| 280 | [Auto-update & the store gate](done/280-auto-update.md) | `electron-updater` against the release feed the Release workflow already publishes: background download, restart only with consent (through the normal unsaved-project guard), and a **Settings ▸ About** tab to check on demand and opt into checking at launch — off by default, since a check is an outbound call. One runtime `store-build.js` gate turns the whole feature off in a Mac App Store build, where the store does the updating | — |
| 290 | [Wire-riding part drags](done/290-wire-riding-part-drags.md) | **Option-drag** a seated part and every wire end in a node one of its pins occupies moves with it, keeping its row and offset — the far ends stay put and the drop is all-or-nothing, so a re-seat can no longer silently rewire the circuit. Shift stays the marquee; Option already means "change what this drag takes with it" | 40, 50, 110, 130, 200 |
| 340 | [Cluster drags](done/340-cluster-drags.md) | A **multi-selection drags as one unit** — marquee or ⌘-click several parts, grab any of them, and the whole arrangement travels by one rigid delta; **Option** carries everything riding any member: the wires, and the **legs of the resistors and LEDs plugged into them** (one leg riding bends the part around the other, both riding translates it whole). All-or-nothing (one member, wire end or leg with nowhere to land reddens the lot) and one undo step. A board in the set refuses the press, since strips have their own drag. Restates the ride rule per PIN rather than per anchor, which is what finally lets a rail-anchored LED or resistor carry its wiring | 40, 50, 110, 200, 240, 290 |
| 330 | [Shared memory blobs & the image's source file](330-shared-memory-blobs.md) | A project file's ROM bytes move into a **content-addressed `blobs` table** every desktop shares: one copy of an image however many chips hold it, and two copies exactly when the bytes differ — so the same file loaded twice with an edit in between is stored as the two images it is. Each chip also remembers **the file it was loaded from**, shown in the memory inspector and on its Properties card in place of the opaque `<guid>.bin` cache path | 180, 190, 250 |
| 320 | [AI desk review](320-ai-desk-review.md) | A **Review** mode in the AI panel: it reads the desktop you already have, runs the app's own engine-backed checks over it (floating inputs on gates that are in use, a tri-state part switched off, two outputs on a net, shorts, oscillation, an LED with nothing limiting it), and asks the model to explain what they mean. Chip Hippo finds the faults and the model explains them — never the other way round. It changes nothing, so unlike Build it is not locked out while the simulation runs | 70, 90, 140, 260 |
